...
[ブラウザ版を今すぐ使う](https://crownecoyah.github.io/yofukashi-clock-25/pwa/) — インストール不要の時計と予定管理。端末内保存、25時記法、ICS書き出し、基本的なオフライン動作に対応します。

[PWA版の使い方・開発方法](public/pwa/README.md) · [実装・検証報告](PWA-REPORT.md)

## iOS版開発協力者を募集しています

macOS版のSwift／SwiftUI実装と25時記法の変換ロジック・テストを出発点に、iOS版を一緒に検討してくださる方を募集しています。MIT Licenseで利用・改変できます。

最初から全機能を完成させる必要はなく、完成版を一人で無償制作することは参加条件ではありません。一部分だけの協力、設計相談・提案、コードレビューも歓迎します。仕様説明、テスト、フィードバックはプロジェクト側が担当します。

貢献者はご本人の希望する名前・リンクを確認し、iOS版開発者としてREADMEとサイトに掲載します。App Store公開は未決定で、Apple Developerアカウント、費用、保守体制は参加者と相談して決めます。

[開発計画](IOS_PORT.md) · [GitHubで相談する](https://github.com/crownecoyah/yofukashi-clock-25/issues/1) · [貢献ガイド](CONTRIBUTING.md)
...
## 初回起動・macOSにブロックされた場合

1. ZIPを展開する。
2. 「夜ふかし時計25時.app」をアプリケーションフォルダへ移動する。
3. アプリを一度開こうとする。
4. macOSにブロックされた場合は「システム設定」を開く。
5. 「プライバシーとセキュリティ」を開く。
6. 下へスクロールし、「このまま開く」を選択する。
7. 再表示された確認画面で「開く」を選択する。

> 本アプリはDeveloper ID署名およびAppleによる公証を受けていません。配布元とSHA-256を確認し、信頼できる場合に限って実行してください。

※ ZIPを展開する前に、Releaseの `SHA256SUMS.txt` とダウンロードしたZIPのSHA-256を照合してください。

[Appleの案内：Macでアプリを安全に開く](https://support.apple.com/ja-jp/102445)
